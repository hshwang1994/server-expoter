...
## 역할 설명
3-channel JSON 출력 스키마 관리. `sections.yml` (10 섹션) + `field_dictionary.yml` (39 Must + 20 Nice + 6 Skip = 65 entries) + `baseline_v1/` (벤더별 회귀 기준선) + `callback_plugins/json_only.py` (출력 envelope) 일관성 보장. fragment 정규화 → 표준 JSON 조립 파이프라인 (build_*.yml 5종).

## 주요 영역
- `schema/sections.yml` — 10 섹션 정의 (system, hardware, bmc, cpu, memory, storage, network, firmware, users, power)
- `schema/field_dictionary.yml` — 필드 사전 (Must 39 + Nice 20 + Skip 6 = 65 entries)
...
- **Field Dictionary 39 Must**: Must 필드는 모든 vendor baseline에 존재해야 함. Nice는 vendor-specific 허용. Skip은 의도적 미수집. (cycle-018 실측 2026-05-01 = 39 Must / 20 Nice / 6 Skip)
