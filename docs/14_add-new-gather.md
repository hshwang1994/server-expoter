...
> **이 문서는** server-exporter 에 새로운 수집 채널 (예: IPMI / SNMP) 또는 새로운 섹션 (예: thermal / GPU) 을 추가하는 개발자를 위한 표준 절차서다.
>
> 본 절차를 그대로 따르면 fragment 철학을 위반하지 않고, 기존 3 채널 (OS / ESXi / Redfish) 에 영향을 주지 않으면서 새 수집 항목을 안전하게 붙일 수 있다.
>
> 시작하기 전에 06 (gather 구조), 07 (normalize 흐름), 10 (adapter 시스템) 세 문서를 먼저 읽기를 권장한다.
